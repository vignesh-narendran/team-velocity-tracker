...
router.put('/', upload.single('logo'), async (req, res) => { const { companyName, accentColor, roles, sprintStatuses, projectStatuses, complexities, storyStages, spToHours, ignoreLeaves } = req.body; let setting = await prisma.setting.findFirst(); const updateData: any = {}; if (companyName) updateData.companyName = companyName; if (accentColor) updateData.accentColor = accentColor; if (roles) updateData.roles = roles; if (sprintStatuses) updateData.sprintStatuses = sprintStatuses; if (projectStatuses) updateData.projectStatuses = projectStatuses; if (complexities) updateData.complexities = complexities; if (storyStages) updateData.storyStages = storyStages; if (spToHours) updateData.spToHours = parseInt(spToHours); if (ignoreLeaves) updateData.ignoreLeaves = ignoreLeaves === 'true'; if (req.file) updateData.logoUrl = '/uploads/' + req.file.filename; if (setting) setting = await prisma.setting.update({ where: { id: setting.id }, data: updateData }); else setting = await prisma.setting.create({ data: updateData }); res.json(setting); });
